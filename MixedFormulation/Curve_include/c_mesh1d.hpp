		...
		) 
{
	...
	while (bgeot::read_until(ist, "BEGIN_ARC")) {
		...
		scalar_type lx_temp = lpoints[1][0]-lpoints[0][0];
		scalar_type ly_temp = lpoints[1][1]-lpoints[0][1];
		scalar_type lz_temp = lpoints[1][2]-lpoints[0][2];
		scalar_type lnorm = sqrt(lx_temp*lx_temp+ly_temp*ly_temp+lz_temp*lz_temp);	
		Curv.push_back(0.0);
		lx.push_back(lx_temp/lnorm);
		ly.push_back(ly_temp/lnorm);
		lz.push_back(lz_temp/lnorm);
