...
c_problem3d1d::assembly_rhs(void)
{
	...
	asm_network_bc(Mvv, Fv, 
			mimv, mf_Uvi, mf_coefv, BCv, P0, c_param.R(), bcoef);
	gmm::add(Mvv, 
		gmm::sub_matrix(AM,
			gmm::sub_interval(dof.Ut()+dof.Pt(), dof.Uv()),
			gmm::sub_interval(dof.Ut()+dof.Pt(), dof.Uv())));
...
c_problem3d1d::assembly_nonlinear_mat(void)
{
	...
	for(size_type i=0; i<nb_branches; ++i){ 
		...
		gmm::add(gmm::scaled(NMvvi,-1.0), 
			gmm::sub_matrix(AM, 
				gmm::sub_interval(dof.Ut()+dof.Pt()+shift, dofi), 
				gmm::sub_interval(dof.Ut()+dof.Pt()+shift, dofi))); 
	...
	asm_network_nonlinear_junctions(NLJvv, 
		...
		c_param.Gamma(),c_param.R(), c_param.G(), (c_param.Curv()),Uv_old); 
...
void
c_problem3d1d::deassebly_nonlinear_mat(void)
{
	gmm::add( gmm::scaled(NLJvv,-1.0), 
		gmm::sub_matrix(AM, 
			gmm::sub_interval(dof.Ut()+dof.Pt(), dof.Uv()), 
			gmm::sub_interval(dof.Ut()+dof.Pt(), dof.Uv())  )); 
...
{
	cout<<" Solving the system with a Fixed Method..."<<endl;
	...
	{   
		cout<<"Solving iter="<<N_iter<<endl;
		if(N_iter>1){
			cout<<"  Deassembly nonlinear term at iter "<<N_iter<<endl;
			deassebly_nonlinear_mat();
		}
		...
		for(size_type b=0; b<nb_branches; b++){
			...
			Error+=asm_L2_dist(mimv, 
							...
							mf_Uvi[b] , gmm::sub_vector(Uv_old,gmm::sub_interval(dof.Ut()+dof.Pt()+shift, dofi)), 
							meshv.region(b) );
		}
		cout<<"  Error done at iter "<<N_iter<<": "<<Error<<"  with minError "<<minERR <<endl<<endl;
	...
	time=gmm::uclock_sec() - time;

	if(Error>minERR)
	{
		cout<<endl<<"  The method doesn't converged for minumum Error="<<minERR<<endl;
		cout<<"      Error done:      ERROR="<<Error<<endl<<endl;
	} else{
		cout<<"The converged in "<<N_iter<<" iterations with L2 error "<<Error<<endl;
	}

	cout << "Time to solve the fixed method : " << time << " seconds"<<endl<<endl;
}
